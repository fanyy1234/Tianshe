...
public class OrderListMarketFragment extends BaseFragment {
...
//                        startActivity(intent);
    ...
    private void cancelOrder(int p){
        showLoadingDialog(0);
        Call<ResultDO> call=AppClient.getAppAdapter().cancelOrder(dataSet.get(p).getId());
...
//        });
    ...
    private void getOrderList(){
        Call<ResultDO<List<Order>>> call = AppClient.getAppAdapter().getMarketOrder(memberId,
                pageNo,status, Constants.PAGE_SIZE);
